...
*/
async function tankUser(guildId, userToTank, author, reason, duration, uom, allowTankingStaff) {
    if (arguments.length < 7) {
        allowTankingStaff = false;
    }
    ...
    try {
        ...
        for (roleIcannotTank of rolesICannotTank) {
            if (!allowTankingStaff && (userToTank._roles.includes(roleIcannotTank) || userToTank._roles.includes(CONFIG.servers[guildId].modUserRole) || userToTank._roles.includes(CONFIG.servers[guildId].botUserRole) || userToTank._roles.includes(CONFIG.servers[guildId].botMasterRole) || userToTank.permissions.has(1 << 3))) {
        ...
        memberDisconnected = await SERVICES.guildService.disconnectMemberFromVC(userToTank.id);
