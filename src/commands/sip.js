...
async function handle(message) {
	...
	if ((message.channel.id === CONFIG.servers[message.guild.id].tankChannel) || (message.channel.id === CONFIG.servers[message.guild.id].minorChannel)) {
		return;
	}
	var msgString;
	if (action === "old_sip") {
	...
	if (action === "mysips") {
		var userObj = await SERVICES.persistenceService.getSipCountForUser(guild, userId);
		if (userObj !== undefined) {
			msgString = mention + " -";
			Object.keys(userObj).forEach((s) =>{
				if ((s !== 'userId') && (s !== 'lastSip')) {
					msgString += " " + s + ": " + userObj[s];
				}
			});
		} else {
			msgString = mention + ", no time like the present to start now!";
		}
	} else {
		...
		if ((userObj.slow !== undefined) && (userObj.slow)) {
			...
			else if ((userObj.count == 3616) && (action == "sip")) {
					msgString = "For Flush so loved the sips, he gave his only begotten beer mug more beer to sip from ~ Sips 3616";
			}
			else {
