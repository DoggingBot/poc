async function handle(invite, old) {
  var cfg = CONFIG.servers[invite.guild.id];
	...
	if (old) {
		...
		let db_invite = await SERVICES.inviteService.getInvites(cfg.serverID); // DB alteration ID => Id
		...
		refreshedInvite = await SERVICES.inviteService.updateInvite(cfg.serverID, db_invite, true); // DB alteration ID => Id
		...
		await SERVICES.inviteService.createInvite(cfg.serverID, invite); // DB alteration ID => Id
